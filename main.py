...
async def webapp_data_handler(message: types.Message, bot: Bot):
    ...
    # Recalculate totals on server for safety
    total = 0.0
    lines = []
    for it in items:
    ...
    if discount_pct:
        user_text_parts.append(f"Скидка {discount_pct}%: -{discount_amount:.2f}₽")
    user_text_parts.append(f"Итого к оплате: {final_total:.2f}₽")
    user_text_parts.extend([
        "",
        f"Адрес: {address}",
        f"Телефон: {phone}",
        f"Оплата: {'Карта' if payment == 'card' else 'Наличные'}",
    ])
    ...
    if MANAGER_CHAT_ID:
        ...
        if discount_pct:
            manager_text_parts.append(f"Скидка {discount_pct}%: -{discount_amount:.2f}₽")
        manager_text_parts.append(f"Итого: {final_total:.2f}₽")
        try:
